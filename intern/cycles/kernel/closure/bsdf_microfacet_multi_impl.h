 ...
 */
ccl_device float3 MF_FUNCTION_FULL_NAME(mf_eval)(const float3 wi, const float3 wo, const bool wo_outside, const float3 color, const float alpha_x, const float alpha_y, const uint seed
...
)
{
	...
	for(int order = 0; order < 10; order++) {
		...
		if(!mf_sample_height(wr, hr, C1_r, G1_r, lambda_r, lcg_step_float(&rng)))
			break;
		float3 wm = mf_sample_vndf(-wr, alpha, make_float2(lcg_step_float(&rng), lcg_step_float(&rng)));
...
#ifdef MF_MULTI_GLASS
			bool next_outside;
			wr = mf_sample_phase_glass(-wr, outside? eta: 1.0f/eta, wm, lcg_step_float(&rng), &next_outside);
			if(!next_outside) {
				outside = !outside;
				wr = -wr;
				hr = -hr;
...
#elif defined(MF_MULTI_DIFFUSE)
			wr = mf_sample_phase_diffuse(wm, lcg_step_float(&rng), lcg_step_float(&rng));
#else /* MF_MULTI_GLOSSY */
 ...
 */
ccl_device float3 MF_FUNCTION_FULL_NAME(mf_sample)(float3 wi, float3 &wo, const float3 color, const float alpha_x, const float alpha_y, const uint seed
#ifdef MF_MULTI_GLASS
	, const float eta
#elif defined(MF_MULTI_GLOSSY)
	, float3 *n, float3 *k
